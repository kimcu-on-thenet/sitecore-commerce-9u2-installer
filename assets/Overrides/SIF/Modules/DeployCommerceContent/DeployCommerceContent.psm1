    ...
    try {       
        switch ($Name) {
            {($_ -match "CommerceOps")} {                    
				...
				# {
					Write-Host "KIM-Json Path"
					$sitecoreIdentityServerUrl = "https://localhost:5050$ServicePortSuffix"
					$originalJson.AppSettings.SitecoreIdentityServerUrl = $sitecoreIdentityServerUrl
					
					$sslPort = "5000$ServicePortSuffix"
					$originalJson.AppSettings.SslPort = $sslPort
				
                    $allowedOrigins = @("https://localhost:4200$ServicePortSuffix", "https://$SiteHostHeaderName")
				...
				foreach ($p in $originalJson.Policies.'$values') {
					if ($p.'$type' -eq 'Sitecore.Commerce.Plugin.SQL.EntityStoreSqlPolicy, Sitecore.Commerce.Plugin.SQL') {						
						...
						Write-Host "Replacing in MigrationSqlPolicy $oldServer with $CommerceServicesDbServer and $oldDatabase with $CommerceServicesGlobalDbName"
					}
				}
				...
				foreach ($jsonFile in $environmentFiles) {
					...
					foreach ($p in $json.Policies.'$values') {
						if ($p.'$type' -eq 'Sitecore.Commerce.Plugin.SQL.EntityStoreSqlPolicy, Sitecore.Commerce.Plugin.SQL') {
							...
							$Writejson = $true
							$p.ServiceUrl = "$oldServiceUrl$ServicePortSuffix"
							Write-Host "Replacing in EnvironmentBusinessToolsPolicy $oldServiceUrl with $oldServiceUrl$ServicePortSuffix"
						} elseif ($p.'$type' -eq 'Plugin.Sample.Customers.CsMigration.ProfilesSqlPolicy, Plugin.Sample.Customers.CsMigration') {
							$oldServer = $p.Server
							$Writejson = $true
							$p.Server = $CommerceServicesDbServer
							Write-Host "Replacing in ProfilesSqlPolicy $oldServer with $CommerceServicesDbServer"
						} elseif ($p.'$type' -eq 'Sitecore.Commerce.Plugin.Management.SitecoreConnectionPolicy, Sitecore.Commerce.Plugin.Management') {
							# if ($SiteHostHeaderName -ne "sxa.storefront.com") 
							# {
								$oldHost = $p.Host;
            ...
            {($_ -match "CommerceShops") -or ($_ -match "CommerceAuthoring") -or ($_ -match "CommerceMinions")}  {
				...
				# {
                    ...
                    $allowedOrigins = @("https://localhost:4200$ServicePortSuffix", "http://localhost:4200$ServicePortSuffix", "https://$SiteHostHeaderName")
                    $originalJson.AppSettings.AllowedOrigins = $allowedOrigins 
                # }
            ...
            'SitecoreIdentityServer' {
				...
				#### Support multiple instances
				$uris = @("http://localhost:4200$ServicePortSuffix", "http://localhost:4200$ServicePortSuffix/?", "https://localhost:4200$ServicePortSuffix", "https://localhost:4200$ServicePortSuffix/?")
				$originalJson.AppSettings.Clients[0].RedirectUris = $uris
				$originalJson.AppSettings.Clients[0].PostLogoutRedirectUris = $uris
				$originalJson.AppSettings.Clients[0].AllowedCorsOrigins = $uris
				
				$sslPort = $originalJson.AppSettings.SslPort
				$originalJson.AppSettings.SslPort = "$sslPort$ServicePortSuffix"
			...
			'SitecoreBizFx' {
                ...
                if ($CommerceAuthoringServicesPort -ne "5000") {
					...
					#### Support multiple instances
					$oldIdentityServerUri = $originalJson.IdentityServerUri
					$originalJson.IdentityServerUri = "$oldIdentityServerUri$ServicePortSuffix"
					Write-Host "Replacing SitecoreBizFx IdentityServerUri $oldIdentityServerUri with $oldIdentityServerUri$ServicePortSuffix"
					
					$oldBizFxUri = $originalJson.BizFxUri
					$originalJson.BizFxUri = "$oldBizFxUri$ServicePortSuffix"
					Write-Host "Replacing SitecoreBizFx BizFxUri $oldBizFxUri with $oldBizFxUri$ServicePortSuffix"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU0cb1FocYGZLItDEnFWh5Ub/A
# LAqgghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFBacJGzSu4xFWzkIi/6SbDXH
# 03lpMA0GCSqGSIb3DQEBAQUABIIBAG1v1iEb55nnVryQXMSYXeb7VgONc+u/m0+q
# AUstU9JTDFy5GBw/9myNMPMdYNk6C1piA6l8o2DdVGj4TsdNfIjX72Pv3r1hNH04
# jedeRRMImgM/SLXTmtRCaSu+bH3labSflxlLbn4GGVzmeOGeuMOozsfpH/ftMOFw
# iAoLiilr5oQolnexxyYpg9zNZXva99ULrdwohQ290wTw9OkOyStvJU+Y2OBa+T3x
# DOclK05DaiR+PnUDdfRBV37T9kyLclG+e7tegMKj/mCR5BkMhvM1w9h/vSqFToTo
# r8mkUJjLZc18uKWwmBPId6BGajI4XroBdWZwpjDIY0kkcq3rOlChggILMIICBwYJ
...
# MTgwNjE0MTc1MTE0WjAjBgkqhkiG9w0BCQQxFgQUptnq9MyN0MCC140anSmKzBm3
# GYUwDQYJKoZIhvcNAQEBBQAEggEAoWDyxJcn1rk8mi5dIruC6ImQlxc3crCmyvuj
# 2Fu+PyDX/f/RoRCTQj+Bw9BOcPRcAITPbWWVWndgBCO468O9HqGml/oRX4qEANW7
# UKU/enQyTaDGnrfrk6VLRDSOBdMFoe+qIimhd867WAjNciS5leD9LSYCEyXDIClP
# sAP1gKKxYXvilXZ68fsWYjZzLLINBh03NeUIZXEB1r4s+S7jZyN7Mbwfk77IeByA
# WVijCLoShkV5fTqG/tfFIkbyKh2wi++/Q9c3OT+LLD/9FOcL0qsg/jnczttelsfr
# 5xH2W+lkJKzes9UDrJUIRU7tZMH1pTSDOWjTaBqFaMIi8pJ28w==
