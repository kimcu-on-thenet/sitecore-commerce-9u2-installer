...
param(
	...
	[string] $InstanceName = "sc92commerce",
	[Parameter(Mandatory=$true)]
	[string] $SiteName = "sc92commerce.dev.local",
	[Parameter(Mandatory=$true)]
	[string] $XConnectSiteName = "sc92commerce_xconnect.dev.local",
	[Parameter(Mandatory=$true)]
	[string] $SiteHostHeaderName = "sxa.storefront.com",	
	[string] $CommerceSearchProvider = "SOLR",
	[Parameter(Mandatory=$true)]
	[string] $WebInstallDir,
	[Parameter(Mandatory=$true)]
	[string] $ServicesPortSuffix = "1",
	[Parameter(Mandatory=$true)]
	[string] $SqlServer,
	[Parameter(Mandatory=$true)]
	[string] $CommerceServicesGlobalDbName = "$($InstanceName)_SitecoreCommerce9_Global",
	[Parameter(Mandatory=$true)]
	[string] $CommerceServicesDbName = "$($InstanceName)_SitecoreCommerce9_SharedEnvironments",
	[Parameter(Mandatory=$true)]
	[string] $CommerceEngineCertificatePath,
	[Parameter(Mandatory=$true)]
	[string] $SolrRoot = "",
	[Parameter(Mandatory=$true)]
	[string] $ResourceInstallPath = "",
	[Parameter(Mandatory=$true)]
	[string] $SitecoreModulesPath = "",
	[Parameter(Mandatory=$true)]
	[string] $BizFxFolder = "",
	[Parameter(Mandatory=$true)]
...
$InstallDir = Join-Path -Path $WebInstallDir -ChildPath $SiteName | Resolve-Path
$XConnectInstallDir = Join-Path -Path $WebInstallDir -ChildPath $XConnectSiteName | Resolve-Path

$PowerShellExtensionsModuleFullPath = Join-Path -Path $SitecoreModulesPath -ChildPath "Sitecore PowerShell Extensions*.zip" | Resolve-Path

$SXAModuleFullPath = Join-Path -Path $SitecoreModulesPath -ChildPath "Sitecore Experience Accelerator*.zip" | Resolve-Path

$SitecoreBizFxServicesContentPath = Join-Path -Path ".." -ChildPath $BizFxFolder | Resolve-Path

Resolve-Path -Path "..\Sitecore.Commerce.Engine.*.zip" | Write-Host

$params = @{
		CommerceServicesPostfix = $InstanceName													##################
        Path = Resolve-Path ".\Configuration\Commerce\$($MasterSingleServer)"	
		SiteName = $SiteName
		SiteHostHeaderName = $SiteHostHeaderName 
		InstallDir = $InstallDir
		XConnectInstallDir = $XConnectInstallDir
		CertificateName = $SiteHostHeaderName													##################
		CommerceServicesDbServer = $SqlServer    												##################
		...
		SearchIndexPrefix = "$($InstanceName)_"
		AzureSearchServiceName = ""
		AzureSearchAdminKey = ""
		AzureSearchQueryKey = ""
		CommerceEngineDacPac = Resolve-Path -Path "..\Sitecore.Commerce.Engine.DB.dacpac"	  	################## 
		CommerceOpsServicesPort = "5015$($ServicesPortSuffix)"
		CommerceShopsServicesPort = "5005$($ServicesPortSuffix)"
		CommerceAuthoringServicesPort = "5000$($ServicesPortSuffix)"
		CommerceMinionsServicesPort = "5010$($ServicesPortSuffix)"
		ServicePortSuffix = $ServicesPortSuffix													##################
		SitecoreCommerceEngineZipPath = Resolve-Path -Path "..\Sitecore.Commerce.Engine.2*.zip"	##################	
		SitecoreBizFxServicesContentPath = $SitecoreBizFxServicesContentPath					##################
		SitecoreIdentityServerZipPath = Resolve-Path -Path "..\Sitecore.IdentityServer.1.*.zip"
		CommerceEngineCertificatePath = Resolve-Path $CommerceEngineCertificatePath				##################
        ...
        PowerShellExtensionsModuleFullPath = $PowerShellExtensionsModuleFullPath				##################
        SXAModuleFullPath = $SXAModuleFullPath													##################
		...
		SitecoreIdentityServerName = "SitecoreIdentityServer_$($InstanceName)"
		InstallationDir = $WebInstallDir														##################
    }
...
Remove-Logs -FromPath $DEPLOYMENT_DIRECTORY
